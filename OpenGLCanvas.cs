...
namespace LayerViewer
{
    public class OpenGLCanvas
    {
        ...
        public OpenGLCanvas(GLControl gl, Form parent)
        {
            _gl = gl;
            _parent = parent;
            Init();
        ...
        protected OpenTK.Vector3 _camera = new OpenTK.Vector3(0f, 0, 1.25f);
        protected OpenTK.Vector3 _unmovedCamera = new OpenTK.Vector3(0, 0, 1f);
        public void Move(float deltaX, float deltaY)
        {
            ...
            Console.WriteLine("X: " + deltaX);
            Console.WriteLine("Y: " + deltaY);
            var test = new OpenTK.Vector4(2f * deltaX / 1.678f / 1564f, 2f * deltaY / 1.678f / 825f, 0, 1);
            //test = test * _projection ;

            if (_isMoving)
            {
                var deltaZ = _camera.Z - _vertices[0].Position.Z;
                var deltaXInPicture = (2 * deltaX / _gl.Width) * Math.Tan(_fieldOfView / 2) * deltaZ * _aspectRatio;
                var deltaYInPicture = (2 * deltaY / _gl.Height) * Math.Tan(_fieldOfView / 2) * deltaZ;
        ...
        protected void TargetCenter()
        {
            ...
            _targetCenter.X = (maxX - minX) / 2;
            _targetCenter.Y = (maxY - minY) / 2;

            //Move(_targetCenter.X, _targetCenter.Y);

            //_view = Matrix4.LookAt(new OpenTK.Vector3(0, 0, 1), new Vector3(), OpenTK.Vector3.UnitY);
            //_view = Matrix4.LookAt(new OpenTK.Vector3(0, 0, 1), _targetCenter, OpenTK.Vector3.UnitY);
        }
